...
```text
{webUiUrl}/app/public/published/{external_id}
```

The route `src/webui/app/routes/app/public/published/$externalId.tsx` loads the
publish entity as a guest, inspects `owner.the_type`, and **redirects** to a
typed public route (for example `/app/public/published/person/{externalId}` or
`/app/public/published/doc/dirtree/{externalId}/{dirRefId}` for directories).
...
`src/webui/app/rendering/published-loader.server.ts` so guests see 404 for
...
`src/webui/app/routes/app/public/published/`. Public views:
...
(`/app/public/published/doc/dirtree/{externalId}/{dirId}`) rather than nesting
