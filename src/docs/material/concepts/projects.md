...
from the project, if it has any.
...
A project can declare the other projects it depends on. This is just a
...
Dependencies can be set when a project is created, and changed afterwards
from its properties, via a multi select box listing the other projects in
the workspace.

A project can't depend on itself, and the same project can't be listed
...
Archiving or removing a project drops it from the dependencies of every
