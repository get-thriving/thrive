"""Standard Jupiter response headers (local copy for the MCP service)."""
...
def build_frontdoor_header(
    ...
    """Build the X-Jupiter-FrontDoor header value for MCP requests."""
    return f"{global_properties.version}:{AppShell.MCP.value}:{AppPlatform.MCP.value}:{AppDistribution.MCP.value}"
