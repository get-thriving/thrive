"""The Sync Google user data WebAPI cron."""
...
import jupiter_webapi_sync_google_user_data_do_all.config
import jupiter_webapi_sync_google_user_data_do_all.exceptions
from jupiter_webapi_sync_google_user_data_do_all.config import (
...
async def main() -> None:
    ...
    telemetry_deployment = TelemetryDeployment(
        service="sync-google-user-data-do-all",
    ...
    cron_app_form = JupiterWebApiCronForm.build_from_module_root(
        ...
        telemetry,
        jupiter_webapi_sync_google_user_data_do_all.exceptions,
    )

    rich_print("=" * 80)
    rich_print("Starting Jupiter WebAPI cron (Sync Google user data):")
