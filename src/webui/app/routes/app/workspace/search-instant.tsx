...
function workspaceSearchArgsFromParsedQuery(
  ...
  return {
    ...
    filter_tag_ref_ids:
      query.filterTagRefIds && query.filterTagRefIds.trim() !== ""
        ? query.filterTagRefIds
            .split(",")
            .map((refId) => refId.trim())
            .filter((refId) => refId.length > 0)
        : undefined,
    filter_contact_ref_ids:
      query.filterContactRefIds && query.filterContactRefIds.trim() !== ""
        ? query.filterContactRefIds
            .split(",")
            .map((refId) => refId.trim())
            .filter((refId) => refId.length > 0)
        : undefined,
