...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const result = await apiClient.projects.projectLoad({
      ref_id: id,
      allow_archived: true,
    });
    ...
    return json({
      ...
      allProjects: summaryResponse.projects as Array<ProjectSummary> | null,
    ...
    switch (form.intent) {
      ...
      case "update": {
        ...
        if (result.record_score_result) {
          return redirect(`/app/workspace/apps/projects/${id}`, {
            headers: {
        ...
        return redirect(`/app/workspace/apps/projects`);
      }

      case "create-note": {
        await apiClient.notes.noteCreate({
          owner: noteStdOwner(NamedEntityTag.PROJECT, id),
          content: [],
        });
...
export default function Project() {
  ...
  const projectsByRefId = new Map();
  projectsByRefId.set(loaderData.project.ref_id, loaderData.project);
  ...
  return (
    <LeafPanel
      ...
      entityRefId={loaderData.project.ref_id}
      fakeKey={`project-${loaderData.project.ref_id}`}
      showArchiveAndRemoveButton
      inputsEnabled={inputsEnabled}
      entityArchived={loaderData.project.archived}
      returnLocation={"/app/workspace/apps/projects"}
      ...
      <NestingAwareBlock shouldHide={shouldShowALeaflet}>
          ...
          actions={
            <SectionActions
              ...
              actions={[
                NavSingle({
                  text: "New",
                  link: `/app/workspace/apps/projects/${loaderData.project.ref_id}/milestones/new`,
                }),
              ]}
            />
          }
        >
          <ProjectMilestoneStack milestones={loaderData.milestones} />
        </SectionCard>

        <SectionCard
          id="project-inbox-tasks"
          title="Inbox Tasks"
          actions={
            <SectionActions
              id="project-inbox-tasks"
              ...
              actions={[
                NavSingle({
                  text: "New",
                  link: `/app/workspace/apps/projects/${loaderData.project.ref_id}/inbox-tasks/new`,
                }),
                FilterFewOptionsCompact(
                  "View",
                    ...
                    {EISENS.map((e) => (
                      <Fragment key={e}>
                        ...
                        <InboxTaskKanbanBoard
                          ...
                          cardLinkResolver={(it) =>
                            `/app/workspace/apps/projects/${loaderData.project.ref_id}/inbox-tasks/${it.ref_id}`
                          }
                        />
                      </Fragment>
                    ))}
        ...
        {isWorkspaceFeatureAvailable(
          ...
          <TimeEventInDayBlockStack
            ...
            inputsEnabled={inputsEnabled}
            title="Time Events"
            createLocation={`/app/workspace/calendar/time-event/in-day-block/new-for-project?projectRefId=${loaderData.project.ref_id}`}
        ...
        {isWorkspaceFeatureAvailable(
          ...
          timePlanActivities && (
            <SectionCard
              id="project-time-plans"
              title="Time Plans"
              actions={
                <SectionActions
                  id="project-time-plans-actions"
                  topLevelInfo={topLevelInfo}
                  inputsEnabled={inputsEnabled}
                  actions={[
                    NavSingle({
                      text: "Add",
                      highlight: false,
                      link: `/app/workspace/apps/time-plans/add-project-to-plans?projectRefId=${loaderData.project.ref_id}`,
                    }),
                  ]}
                />
              }
            >
              <TimePlanActivityList
                ...
                projectsByRefId={projectsByRefId}
