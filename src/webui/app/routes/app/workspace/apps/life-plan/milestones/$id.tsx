...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const [summaryResponse, allTags, response] = await Promise.all([
      ...
      apiClient.lifePlan.milestoneLoad({
        ref_id: id,
        allow_archived: true,
      }),
    ]);
  ...
  try {
    switch (form.intent) {
      case "update": {
        ...
        return redirect(`/app/workspace/apps/life-plan/milestones/${id}`);
      }

      case "create-note": {
        await apiClient.notes.noteCreate({
          owner: noteStdOwner(NamedEntityTag.MILESTONE, id),
          content: [],
        });
      ...
      case "archive": {
        ...
        return redirect(`/app/workspace/apps/life-plan/milestones`);
      }
