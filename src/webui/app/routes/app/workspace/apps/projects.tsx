...
export default function Projects() {
  ...
  for (const entry of loaderData.projects as Array<ProjectFindResultEntry>) {
    entriesByRefId.set(entry.project.ref_id, projectFindEntryToParent(entry));
  }

  const sortedProjects = sortProjectsNaturally(
    (loaderData.projects as Array<ProjectFindResultEntry>).map(
      (b) => b.project,
    ),
  ).filter((bp) => {
  ...
  // Shared projects may reference aspects from another workspace. Include
  ...
  const projectMilestonesByRefId = new Map<string, ProjectMilestone[]>(
    (loaderData.projects as Array<ProjectFindResultEntry>).map((b) => [
      b.project.ref_id,
      b.milestones!,
    ]),
  );
  ...
  const ownersByProjectRefId = new Map<string, UserLight>(
    (loaderData.projects as Array<ProjectFindResultEntry>).map((b) => [
      b.project.ref_id,
      b.owner,
    ]),
  ...
  return (
    <TrunkPanel
      key={"projects"}
      createLocation="/app/workspace/apps/projects/new"
                        ...
                        <Fragment key={goal.ref_id}>
                          ...
                          {isBigScreen && (
                            <ProjectTimelineBigScreen
                              ...
                              ownersByProjectRefId={ownersByProjectRefId}
