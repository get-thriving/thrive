...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [projectResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `Project:std:${query.projectRefId}`,
      }),
    ]);
    ...
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateProjectWithPlan({
          project_ref_id: query.projectRefId,
          time_plan_ref_ids: form.targetTimePlanRefIds,
          kind: form.kind,
          feasability: form.feasability,
        });

        return redirect(`/app/workspace/apps/projects/${query.projectRefId}`);
      }
...
export default function AddProjectToPlans() {
  ...
  return (
    <LeafPanel
      ...
      fakeKey={`add-project-to-plans/${searchParams.get("projectRefId")}`}
      returnLocation={`/app/workspace/apps/projects/${searchParams.get("projectRefId")}`}
...
export const ErrorBoundary = makeLeafErrorBoundary(
  (params, searchParams) =>
    `/app/workspace/apps/projects/${searchParams.get("projectRefId")}`,
  ParamsSchema,
  {
    error: () =>
