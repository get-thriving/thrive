...
import { sortProjectsNaturally } from "@jupiter/core/apps/projects/root";
import { ProjectCard } from "@jupiter/core/apps/projects/component/card";
import {
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    if (!timePlanAllowsInboxTasks(timePlanResult.time_plan)) {
      throw new Response(
        "Project tasks can only be added to daily or weekly time plans",
        { status: 400 },
      );
    }

    const projects = sortProjectsNaturally(
      timePlanResult.target_projects ?? [],
    );
    const projectRefIds = projects.map((bp) => bp.ref_id);
    ...
    const inboxTasksResult =
      projectRefIds.length > 0
        ? await apiClient.inboxTasks.inboxTaskFind({
            ...
            filter_source_entity_ref_ids: projectRefIds,
          })
        : { entries: [] };
...
export default function TimePlanAddFromIncludedProjectTasks() {
  const { id } = useParams();
  ...
  const tasksByProjectRefId = new Map<string, InboxTask[]>();
  for (const inboxTask of filteredInboxTasks) {
    if (parentLinkNamespaceFromEntityLinkWire(inboxTask.owner) !== PROJECT) {
      continue;
    }
    const projectRefId = entityLinkRefIdFromWire(inboxTask.owner);
    const existing = tasksByProjectRefId.get(projectRefId) ?? [];
    existing.push(inboxTask);
    tasksByProjectRefId.set(projectRefId, existing);
  }

  const projectsWithTasks = loaderData.projects.filter(
    (project) => (tasksByProjectRefId.get(project.ref_id) ?? []).length > 0,
  );
  ...
  return (
    <LeafPanel
      key={`time-plan-${id}/add-from-included-project-tasks`}
      fakeKey={`time-plan-${id}/add-from-included-project-tasks`}
      returnLocation={withTimePlanView(
        `/app/workspace/apps/time-plans/${id}`,
        timePlanViewParam,
      )}
      returnLocationDiscriminator="add-from-included-project-tasks"
    ...
    >
      ...
      <SectionCard
        id="time-plan-included-project-tasks"
        title="Included Project Tasks"
        actions={
          <SectionActions
            id="time-plan-add-from-included-project-tasks"
        ...
        {projectsWithTasks.length === 0 ? (
          <Typography sx={{ mt: 2 }}>
            {loaderData.projects.length === 0
              ? "No projects are on this time plan yet."
              : "No project tasks available to add."}
          </Typography>
        ) : (
          <EntityStack>
            {projectsWithTasks.map((project) => {
              const tasks = tasksByProjectRefId.get(project.ref_id) ?? [];
              return (
                <Fragment key={`project-group-${project.ref_id}`}>
                  <ProjectCard
                    topLevelInfo={topLevelInfo}
                    project={project}
                    linksEnabled={false}
