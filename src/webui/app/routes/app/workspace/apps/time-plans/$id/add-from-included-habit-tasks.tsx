...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    if (!timePlanAllowsInboxTasks(timePlanResult.time_plan)) {
      throw new Response(
        "Habit tasks can only be added to daily or weekly time plans",
        { status: 400 },
      );
    }

    const habits = sortHabitsNaturally(timePlanResult.target_habits ?? []);
    const habitRefIds = habits.map((habit) => habit.ref_id);

    const inboxTasksResult =
      habitRefIds.length > 0
        ? await apiClient.inboxTasks.inboxTaskFind({
            allow_archived: false,
            filter_just_workable: true,
            filter_namespace: [HABIT],
            filter_source_entity_ref_ids: habitRefIds,
          })
        : { entries: [] };
...
export default function TimePlanAddFromIncludedHabitTasks() {
  ...
  const tasksByHabitRefId = new Map<string, InboxTask[]>();
  for (const inboxTask of filteredInboxTasks) {
    if (parentLinkNamespaceFromEntityLinkWire(inboxTask.owner) !== HABIT) {
      continue;
    }
    const habitRefId = entityLinkRefIdFromWire(inboxTask.owner);
    const existing = tasksByHabitRefId.get(habitRefId) ?? [];
    existing.push(inboxTask);
    tasksByHabitRefId.set(habitRefId, existing);
  }

  const habitsWithTasks = loaderData.habits.filter(
    (habit) => (tasksByHabitRefId.get(habit.ref_id) ?? []).length > 0,
  );
  ...
  return (
    <LeafPanel
      key={`time-plan-${id}/add-from-included-habit-tasks`}
      fakeKey={`time-plan-${id}/add-from-included-habit-tasks`}
      returnLocation={withTimePlanView(
        `/app/workspace/apps/time-plans/${id}`,
        timePlanViewParam,
      )}
      returnLocationDiscriminator="add-from-included-habit-tasks"
    ...
    >
      ...
      <SectionCard
        id="time-plan-included-habit-tasks"
        title="Included Habit Tasks"
        actions={
          <SectionActions
            id="time-plan-add-from-included-habit-tasks"
      ...
      >
        ...
        {habitsWithTasks.length === 0 ? (
          <Typography sx={{ mt: 2 }}>
            {loaderData.habits.length === 0
              ? "No habits are on this time plan yet."
              : "No habit tasks available to add."}
          </Typography>
        ) : (
          <EntityStack>
            {habitsWithTasks.map((habit) => {
              const tasks = tasksByHabitRefId.get(habit.ref_id) ?? [];
              return (
                <Fragment key={`habit-group-${habit.ref_id}`}>
                  <EntityCard entityId={`habit-${habit.ref_id}`}>
                    <EntityFakeLink>
