...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const [timePlanResult, choreResult, inboxTasksResult] = await Promise.all([
      ...
      apiClient.inboxTasks.inboxTaskFind({
        ...
        filter_source_entity_ref_ids: [query.choreRefId],
      }),
    ]);
    ...
    switch (form.intent) {
      case "gen": {
        await apiClient.gen.genDo({
          ...
          filter_chore_ref_ids: [query.choreRefId],
        });

        return redirect(
          withTimePlanView(
            `/app/workspace/apps/time-plans/${id}/add-from-chore-inbox-tasks?${new URLSearchParams(
              {
                choreRefId: query.choreRefId,
                timePlanActivityRefId: query.timePlanActivityRefId,
              },
...
export default function TimePlanAddFromChoreInboxTasks() {
  const { id } = useParams();
  ...
  return (
    <LeafPanel
      key={`time-plan-${id}/add-from-chore-inbox-tasks`}
      fakeKey={`time-plan-${id}/add-from-chore-inbox-tasks`}
      returnLocation={returnLocation}
      returnLocationDiscriminator="add-from-chore-inbox-tasks"
    ...
    >
      ...
      <SectionCard
        id="time-plan-chore-inbox-tasks"
        title={`Inbox Tasks for ${loaderData.chore.name}`}
        actions={
          <SectionActions
            id="time-plan-add-from-chore-inbox-tasks"
      ...
      >
        ...
        <StandardDivider title="Chore Inbox Tasks" size="large" />
        <InboxTaskList
          topLevelInfo={topLevelInfo}
