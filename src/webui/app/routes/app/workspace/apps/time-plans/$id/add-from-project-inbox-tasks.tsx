...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const [timePlanResult, projectResult, inboxTasksResult] = await Promise.all(
      [
        ...
        apiClient.inboxTasks.inboxTaskFind({
          ...
          filter_source_entity_ref_ids: [query.projectRefId],
        }),
      ],
    );
    ...
    return json({
      ...
      projectRefId: query.projectRefId,
...
export default function TimePlanAddFromProjectInboxTasks() {
  const { id } = useParams();
  ...
  return (
    <LeafPanel
      key={`time-plan-${id}/add-from-project-inbox-tasks`}
      fakeKey={`time-plan-${id}/add-from-project-inbox-tasks`}
      returnLocation={returnLocation}
      returnLocationDiscriminator="add-from-project-inbox-tasks"
    ...
    >
      ...
      <SectionCard
        id="time-plan-project-inbox-tasks"
        title={`Inbox Tasks for ${loaderData.project.name}`}
        actions={
          <SectionActions
            id="time-plan-add-from-project-inbox-tasks"
