...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [todoTaskResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      apiClient.todo.todoTaskLoad({
        ref_id: query.todoTaskRefId,
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `TodoTask:std:${query.todoTaskRefId}`,
      }),
    ]);

  return json({
    todoTask: todoTaskResult.todo_task,
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateTodoTaskWithPlan({
          todo_task_ref_id: query.todoTaskRefId,
...
export default function AddTodoToPlans() {
  ...
  return (
    <LeafPanel
      key="add-todo-to-plans"
      fakeKey={`add-todo-to-plans/${searchParams.get("todoTaskRefId")}`}
      returnLocation={`/app/workspace/apps/todos/${searchParams.get("todoTaskRefId")}`}
      returnLocationDiscriminator="add-todo-to-plans"
...
export const ErrorBoundary = makeLeafErrorBoundary(
  (params, searchParams) =>
    `/app/workspace/apps/todos/${searchParams.get("todoTaskRefId")}`,
  ParamsSchema,
  {
    error: () =>
      `There was an error adding the todo to time plans! Please try again!`,
  },
);
