  ...
  try {
    switch (form.intent) {
      case "update": {
        ...
        return redirect(`/app/workspace/account/mcp-key/${id}`);
      }

      case "archive": {
        await apiClient.mcpKey.mCpKeyArchive({ ref_id: id });

        return redirect(`/app/workspace/account/mcp-key`);
      }
...
export default function McpKeyEdit() {
  ...
  return (
    <LeafPanel
      key={`account/mcp-key/${mcpKey.ref_id}`}
      fakeKey={`account/mcp-key/${mcpKey.ref_id}`}
