...
_PURPOSE_RE: re.Pattern[str] = re.compile(r"^[a-zA-Z0-9_-]+$")
    ...
    def _validate(self) -> None:
        ...
        if not _PURPOSE_RE.match(self.purpose):
            raise InputValidationError(
                "Entity link purpose must be alphanumeric with optional hyphen/underscore",
            )

    def __str__(self) -> str:
        """Canonical wire/database form."""
        return f"{self.the_type}:{self.purpose}:{self.ref_id}"
...
class EntityLinkDatabaseEncoder(RealmEncoder[EntityLink, DatabaseRealm]):
    ...
    def encode(self, value: EntityLink) -> RealmThing:
        """Encode to ``{the_type}:{purpose}:{ref_id}`` for the database realm."""
        return str(value)
...
class EntityLinkDatabaseDecoder(RealmDecoder[EntityLink, DatabaseRealm]):
    ...
    def decode(self, value: RealmThing) -> EntityLink:
        """Decode from the canonical string ``{the_type}:{purpose}:{ref_id}``."""
        ...
        the_type, purpose, ref_id_str = value.rsplit(":", 2)
        if not the_type or not ref_id_str or not purpose:
            raise RealmDecodingError(
                f"Expected entity link to have non-empty type, purpose, and ref id but got {value!r}",
            )
        ...
        if not _PURPOSE_RE.match(purpose):
            raise RealmDecodingError(
                f"Expected entity link purpose to be alphanumeric (with optional hyphen/underscore) but got {purpose!r}",
            )
        try:
            return EntityLink(
                the_type=the_type,
