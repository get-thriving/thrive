"""PostgreSQL storage engine generation code."""
...
class PostgresDomainUnitOfWork(DomainUnitOfWork):
    """A Postgres specific unit of work."""

    _realm_codec_registry: Final[RealmCodecRegistry]
    _connection: Final[AsyncConnection]
    _metadata: Final[MetaData]
    _entity_repository_factories: Final[
        dict[type[Entity], type[PostgresEntityRepository[Entity]]]
    ]
    _record_repository_factories: Final[
        dict[type[Record], type[PostgresRecordRepository[Record, object]]]
    ]
    _repository_factories: Final[dict[type[Repository], type[PostgresRepository]]]
    ...
    def __init__(
        ...
        entity_repository_factories: dict[
            type[Entity], type[PostgresEntityRepository[Entity]]
        ],
        record_repository_factories: dict[
            type[Record], type[PostgresRecordRepository[Record, object]]
        ],
        repository_factories: dict[type[Repository], type[PostgresRepository]],
    ) -> None:
        """Constructor."""
...
class _StandardPostgresRootEntityRepository(
    PostgresRootEntityRepository[_RootEntityT],
    RootEntityRepository[_RootEntityT],
...
class _StandardPostgresStubEntityRepository(
    PostgresStubEntityRepository[_StubEntityT],
    StubEntityRepository[_StubEntityT],
...
class _StandardPostgresTrunkEntityRepository(
    PostgresTrunkEntityRepository[_TrunkEntityT],
    TrunkEntityRepository[_TrunkEntityT],
...
class _StandardPostgresCrownEntityRepository(
    PostgresCrownEntityRepository[_CrownEntityT],
    CrownEntityRepository[_CrownEntityT],
...
class PostgresDomainStorageEngine(DomainStorageEngine):
    """An Postgres specific engine."""
        ...
        *module_roots: ModuleType,
        ...
        def extract_entity_repositories(
            ...
            tuple[
                ...
                type[PostgresEntityRepository[Entity]],
            ]
        ]:
            ...
            for _name, obj in the_module.__dict__.items():
                ...
                if not issubclass(obj, EntityRepository):
                    continue
                if not issubclass(obj, PostgresEntityRepository):
                    continue
        ...
        def extract_record_repositories(
            ...
            tuple[
                ...
                type[PostgresRecordRepository[Record, object]],
            ]
        ]:
            ...
            for _name, obj in the_module.__dict__.items():
                ...
                if not issubclass(obj, RecordRepository):
                    continue
                if not issubclass(obj, PostgresRecordRepository):
                    continue
        ...
        def extract_repositories(
            the_module: ModuleType,
        ) -> Iterator[tuple[type[Repository], type[PostgresRepository]]]:
            ...
            for _name, obj in the_module.__dict__.items():
                ...
                if not issubclass(obj, Repository):
                    continue
                if not issubclass(obj, PostgresRepository):
                    continue
            ...
            for entity_type in extract_entities(m):
                ...
                if issubclass(entity_type, RootEntity):
                    entity_repository_factories[entity_type] = type(
                        f"_StandardPostgresRootEntityRepository_{entity_type.__name__}",
                        (_StandardPostgresRootEntityRepository,),
                        {"__init__": _generic_root_init(entity_type)},
                    )
                elif issubclass(entity_type, StubEntity):
                    entity_repository_factories[entity_type] = type(
                        f"_StandardPostgresStubEntityRepository_{entity_type.__name__}",
                        (_StandardPostgresStubEntityRepository,),
                        {"__init__": _generic_stub_init(entity_type)},
                    )
                elif issubclass(entity_type, TrunkEntity):
                    entity_repository_factories[entity_type] = type(
                        f"_StandardPostgresTrunkEntityRepository_{entity_type.__name__}",
                        (_StandardPostgresTrunkEntityRepository,),
                        {"__init__": _generic_trunk_init(entity_type)},
                    )
                elif issubclass(entity_type, CrownEntity):
                    entity_repository_factories[entity_type] = type(
                        f"_StandardPostgresCrownEntityRepository_{entity_type.__name__}",
                        (_StandardPostgresCrownEntityRepository,),
                        {"__init__": _generic_crown_init(entity_type)},
                    )
                else:
                    raise Exception(f"Unknown entity type: {entity_type}")

        return PostgresDomainStorageEngine(
    ...
    async def get_unit_of_work(self) -> AsyncIterator[DomainUnitOfWork]:
        ...
        async with self._sql_engine.begin() as connection:
            yield PostgresDomainUnitOfWork(
...
def _generic_root_init(
    ...
    [
        _StandardPostgresRootEntityRepository[_RootEntityT],
        RealmCodecRegistry,
        AsyncConnection,
        MetaData,
    ],
    ...
    return lambda self, realm_codec_registry, connection, metadata: super(
        _StandardPostgresRootEntityRepository, self
...
def _generic_stub_init(
    ...
    [
        _StandardPostgresStubEntityRepository[_StubEntityT],
        RealmCodecRegistry,
        AsyncConnection,
        MetaData,
    ],
    ...
    return lambda self, realm_codec_registry, connection, metadata: super(
        _StandardPostgresStubEntityRepository, self
    ).__init__(realm_codec_registry, connection, metadata, entity_type=entity_type)
...
def _generic_trunk_init(
    ...
    [
        _StandardPostgresTrunkEntityRepository[_TrunkEntityT],
        RealmCodecRegistry,
        AsyncConnection,
        MetaData,
    ],
    ...
    return lambda self, realm_codec_registry, connection, metadata: super(
        _StandardPostgresTrunkEntityRepository, self
    ).__init__(realm_codec_registry, connection, metadata, entity_type=entity_type)
...
def _generic_crown_init(
    ...
    [
        _StandardPostgresCrownEntityRepository[_CrownEntityT],
        RealmCodecRegistry,
        AsyncConnection,
        MetaData,
    ],
    ...
    return lambda self, realm_codec_registry, connection, metadata: super(
        _StandardPostgresCrownEntityRepository, self
    ).__init__(realm_codec_registry, connection, metadata, entity_type=entity_type)
