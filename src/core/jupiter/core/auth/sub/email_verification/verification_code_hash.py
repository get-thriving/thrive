...
class VerificationCodeHash(SecretValue):
    ...
    def from_plain(plain: VerificationCodePlain) -> "VerificationCodeHash":
        """Build a hashed verification code from a plain verification code."""
        return VerificationCodeHash(_PASSWORD_HASHER.hash(plain.code_raw))

    def check_against(self, plain: VerificationCodePlain) -> bool:
        """Check that the given verification code and this one match."""
        try:
            _PASSWORD_HASHER.verify(self.code_hash_raw, plain.code_raw)
            return True
...
class VerificationCodeHashEventStoreRealmEncoder(
    RealmEncoder[VerificationCodeHash, EventStoreRealm]
):
    """Encode a verification code hash for storage in the Event Store."""
