"""SQLite persistence for :class:`SearchEntityIndexingRecordRepository`."""

from sqlite3 import IntegrityError
from typing import Final, cast

from jupiter.core.common.search.entity_indexing_record import (
...
class SqliteSearchEntityIndexingRecordRepository(
    SqliteRecordRepository[SearchEntityIndexingRecord, tuple[EntityId, str, EntityId]],
    SearchEntityIndexingRecordRepository,
):
    ...
    def _row_to_record(self, row: dict[str, object]) -> SearchEntityIndexingRecord:
        ...
        search_domain_ref_id = dec(EntityId, DatabaseRealm).decode(
    ...
    async def create(
        self, record: SearchEntityIndexingRecord
    ) -> SearchEntityIndexingRecord:
        ...
        try:
            ...
            raise RecordAlreadyExistsError(
                "Search entity indexing map row already exists",
            ) from err
        return record
    ...
    async def save(
        ...
        result = await self._connection.execute(
            ...
            .where(
                ...
                == enc(EntityId, DatabaseRealm).encode(record.search_domain.ref_id)
            )
        ...
        if result.rowcount == 0:
            raise RecordNotFoundError("Search entity indexing map row does not exist")
        return record
    ...
    async def find_all(
        ...
        """Find all map rows for one or more search domains."""
        enc = self._realm_codec_registry.get_encoder
    ...
    async def find_all_for_search_domain_entity_type(
        ...
        """Load all rows for search domain and type."""
        enc = self._realm_codec_registry.get_encoder
        stmt = (
            select(self._table)
            .where(
    ...
    async def load_optional(
        ...
        """Return one row by search domain, type, and entity id."""
        enc = self._realm_codec_registry.get_encoder
        stmt = (
            select(self._table)
            .where(
    ...
    async def upsert(self, record: SearchEntityIndexingRecord) -> None:
        ...
        upsert_stmt = insert_stmt.on_conflict_do_update(
            index_elements=["search_domain_ref_id", "entity_type", "entity_ref_id"],
            set_={
    ...
    async def remove_all_for_search_domain(
        ...
        """Delete all rows for one search domain."""
        enc = self._realm_codec_registry.get_encoder
        await self._connection.execute(
