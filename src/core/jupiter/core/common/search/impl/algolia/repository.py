...
class AlgoliaSearchRepository(SearchRepository):
    ...
    async def search(
        ...
        if filter_tag_ref_ids is not None:
            tag_ref_ids = [tag_ref_id.as_int() for tag_ref_id in filter_tag_ref_ids]
            if len(tag_ref_ids) == 0:
                filter_parts.append("tag_ref_ids:-1")
        ...
        if filter_contact_ref_ids is not None:
            ...
            if len(contact_ref_ids) == 0:
                filter_parts.append("contact_ref_ids:-1")
        ...
        if filter_location_ref_ids is not None:
            ...
            if len(location_ref_ids) == 0:
                ...
                filter_parts.append(
                    "("
                    + " OR ".join(f"location_ref_ids:{rid}" for rid in location_ref_ids)
                    + ")"
                )
