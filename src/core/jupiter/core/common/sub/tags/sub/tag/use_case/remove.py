...
@use_case_args
class TagRemoveArgs(UseCaseArgsBase):
    """TagRemove args."""
...
class TagRemoveUseCase(
    JupiterTransactionalLoggedInMutationUseCase[TagRemoveArgs, None]
):
    """Use case for removing a tag."""
    ...
    async def _perform_transactional_mutation(
        ...
        await uow.get_for(Tag).remove(args.ref_id)
        await progress_reporter.mark_removed(tag)

        # Remove this tag's ref_id from all other entities that reference it (e.g., links)
