...
@use_case_args
class TimeEventInDayBlockCreateForTodoTaskArgs(JupiterCreateLeafSupportEntityArgs):
    """Args."""
...
@use_case_result
class TimeEventInDayBlockCreateForTodoTaskResult(UseCaseResultBase):
    """Result."""
...
@mutation_use_case()
class TimeEventInDayBlockCreateForTodoTaskUseCase(
    JupiterCreateLeafSupportEntityUseCase[
    ...
    async def _perform_transactional_mutation(
        ...
        args: TimeEventInDayBlockCreateForTodoTaskArgs,
    ) -> TimeEventInDayBlockCreateForTodoTaskResult:
        """Execute the command's action."""
        todo_task, owner_workspace_ref_id = await self.load_owner_entity(
            uow, context.user.ref_id, TodoTask, args.todo_task_ref_id
        )
        time_event_domain = await self.load_parent(
            uow, TimeEventDomain, owner_workspace_ref_id
        )

        new_time_event = TimeEventInDayBlock.new_time_event_for_todo_task(
