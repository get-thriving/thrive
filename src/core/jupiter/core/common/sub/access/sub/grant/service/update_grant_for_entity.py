"""Service for updating an access grant and its derived statuses."""
...
class UpdateGrantForEntityService:
    """Update an access grant and propagate the new level to derived statuses."""
    ...
    async def do_it(
        ...
        """Update the grant and all statuses derived from it."""
        if not access_level.is_invitable:
            raise InputValidationError(
                "Grants must use reader, commenter, or writer access level"
            )
        ...
        if grant.access_level == AccessLevel.OWNER:
            raise InputValidationError("Cannot change the owner's access grant")

        if grant.archived:
            raise InputValidationError("Access grant is already removed")

        if grant.access_level == access_level:
            return

        updated_grant = grant.change_access_level(
            ctx, UpdateAction.change_to(access_level)
        )
        await uow.get_for(AccessGrant).save(updated_grant)
        ...
        for status in statuses:
            await status_repository.upsert(
                AccessStatus.new_access_status(
                    ctx,
                    status.access_domain.ref_id,
                    entity=status.entity,
                    user_ref_id=status.user_ref_id,
                    access_level=access_level,
                    reason=status.reason,
                    access_grant_ref_id=status.access_grant_ref_id,
                )
            )
