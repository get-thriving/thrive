...
from jupiter.core.apps.schedule.sub.external_sync_log.external_uid import (
    ScheduleExternalUid,
)
                ...
                for event in all_events:
                    ...
                    if (
                        ...
                        event_name = self._realm_codec_registry.db_decode(
                            ScheduleEventFullDaysName, event["SUMMARY"]
                        )
                        ...
                        # In-day event
                        in_day_name = self._realm_codec_registry.db_decode(
                            ScheduleEventInDayName, event["SUMMARY"]
                        )
                        uid_base = self._realm_codec_registry.db_decode(
                            ScheduleExternalUid, event["UID"].to_ical().decode()
                        )
                        ...
                        if uid not in all_in_day_events_by_external_uid:
                            ...
                            async with (
                                ...
                                schedule_event_in_day = schedule_event_in_day.update(
                                    ctx,
                                    name=UpdateAction.change_to(in_day_name),
                                )
