...
from jupiter.core.apps.big_plans.stats import BigPlanStats
...
class BigPlan(LeafEntity):
    ...
    location_link = OwnsAtMostOne(
        LocationLink, owner=IsEntityLinkStd(NamedEntityTag.BIG_PLAN.value)
    )
    note = OwnsAtMostOne(Note, owner=IsEntityLinkStd(NamedEntityTag.BIG_PLAN.value))
    publish_entity = OwnsAtMostOne(
        PublishEntity, owner=IsEntityLinkStd(NamedEntityTag.BIG_PLAN.value)
    )
