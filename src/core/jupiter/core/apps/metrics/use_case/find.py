...
class MetricFindUseCase(
    ...
    async def _perform_transactional_read(
        ...
        metrics = await self.find_all_entities(
            uow,
            context.user.ref_id,
            Metric,
        ...
        metric_locations_by_ref_id = {
            link.owner.ref_id: link.locations_ref_ids for link in location_links
        }
        all_metric_location_ref_ids = []
        for location_ref_ids in metric_locations_by_ref_id.values():
            all_metric_location_ref_ids.extend(location_ref_ids)
        locations = []
        if all_metric_location_ref_ids:
            locations = await uow.get_for(Location).find_all_generic(
                allow_archived=False,
                ref_id=list(set(all_metric_location_ref_ids)),
            )
        locations_by_ref_id = {loc.ref_id: loc for loc in locations}

        owner_ref_ids_by_metric_ref_id = (
        ...
        owners = await uow.get(UserRepository).find_all_light_by_ref_ids(
            list(set(owner_ref_ids_by_metric_ref_id.values()))
        )
        owners_by_ref_id = {owner.ref_id: owner for owner in owners}

        access_statuses = await uow.get(
            AccessStatusRepository
        ).load_all_for_entities_and_user(metric_owner_links, context.user.ref_id)
        access_status_by_metric_ref_id = {
            ...
            entries=[
                MetricFindResponseEntry(
                    ...
                    locations=[
                        ...
                        for location_ref_id in metric_locations_by_ref_id.get(
                            m.ref_id, []
                        )
                    ...
                    owner=owners_by_ref_id[owner_ref_ids_by_metric_ref_id[m.ref_id]],
                    access_status=access_status_by_metric_ref_id[m.ref_id],
                )
                for m in metrics
            ],
        )
