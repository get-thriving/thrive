"""Guest readonly use case for loading a published project."""
...
@use_case_args
class ProjectLoadPublicArgs(UseCaseArgsBase):
    """ProjectLoadPublic args."""

    external_id: PublishExternalId
    inbox_task_retrieve_offset: int | None


class ProjectLoadPublicUseCase(
    JupiterGuestReadonlyUseCase[ProjectLoadPublicArgs, ProjectLoadResult]
):
    """Load a published project by publish external id."""

    async def _execute(
        self,
        context: JupiterGuestReadonlyContext,
        args: ProjectLoadPublicArgs,
    ) -> ProjectLoadResult:
        ...
        async with self._ports.domain_storage_engine.get_unit_of_work() as uow:
            ...
            if publish_entity.owner.the_type != NamedEntityTag.PROJECT.value:
                raise InputValidationError(
                    "The publish entity does not refer to a project."
                )
            ...
            if project.project_collection.ref_id != project_collection.ref_id:
                raise InputValidationError(
                    "The publish entity does not refer to a workspace project."
                )
