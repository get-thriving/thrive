...
from jupiter.core.apps.projects.service.unlink_dependencies import (
    ProjectUnlinkDependenciesService,
)
...
class ProjectArchiveService:
    ...
    async def do_it(
        ...
        Callers must have already authorized write access to the project via ACL.
        Milestones are archived as children of that project without a separate ACL
        check per milestone.
        """
        if project.archived:
        ...
        await ProjectUnlinkDependenciesService().unlink_dependencies(
            ctx,
            uow,
            progress_reporter,
            project,
