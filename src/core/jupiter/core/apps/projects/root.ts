...
import { compareProjectStatus } from "#/core/apps/projects/status";
...
export function projectFindEntryToParent(
...
export function projectDonePct(
  project: Project,
  ...
  if (projectStats.all_inbox_tasks_cnt === 0) {
    return 10;
  }

  const pct = Math.floor(
    (projectStats.completed_inbox_tasks_cnt /
      projectStats.all_inbox_tasks_cnt) *
      100,
  );
