      ...
      <Table sx={{ tableLayout: "fixed" }}>
        ...
        <TableBody>
          {projects.map((entry) => {
            ...
            const milestones = projectMilestonesByRefId.get(entry.ref_id) ?? [];
            ...
            return (
              <TableRow id={`project-${entry.ref_id}`} key={entry.ref_id}>
                ...
                >
                  {ownersByProjectRefId?.get(entry.ref_id) && (
                    <UserLightChip
                      user={ownersByProjectRefId.get(entry.ref_id)!}
                      currentUserRefId={currentUserRefId}
                    />
                  )}
                  <EntityLink
                    block={allowSelect}
                    to={`/app/workspace/apps/projects/${entry.ref_id}`}
                    singleLine
                  >
                ...
                <TableCell>
                  <Stack direction="row" spacing={0.5}>
                    <ProjectDonePctTag
                      donePct={projectDonePct(
                        entry,
                        projectStatsByRefId.get(entry.ref_id)!,
                      )}
                    />
