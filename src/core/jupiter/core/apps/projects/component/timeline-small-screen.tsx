...
interface ProjectTimelineSmallScreenProps {
  ...
  projectMilestonesByRefId: Map<string, Array<ProjectMilestone>>;
  projectStatsByRefId: Map<string, ProjectStats>;
  ownersByProjectRefId?: Map<string, UserLight>;
...
export function ProjectTimelineSmallScreen({
  ...
  thisYear,
  projects,
  projectMilestonesByRefId,
  projectStatsByRefId,
  ownersByProjectRefId,
  currentUserRefId,
  dateMarkers,
  selectedPredicate,
  allowSelect,
      ...
      <SmallScreenTimelineList>
        ...
        {projects.map((project) => {
          const { leftMargin, width, betterWidth, betterLeftMargin } =
            computeProjectGnattPosition(thisYear, project);

          const milestones = projectMilestonesByRefId.get(project.ref_id) || [];

          return (
            <SmallScreenTimelineLine
              key={project.ref_id}
              selected={allowSelect && selectedPredicate?.(project)}
              onClick={() => allowSelect && onClick?.(project)}
            >
              <TimelineGnattBlob
                isunderlay="true"
              ...
              {ownersByProjectRefId?.get(project.ref_id) && (
                <UserLightChip
                  user={ownersByProjectRefId.get(project.ref_id)!}
                  currentUserRefId={currentUserRefId}
                />
              )}
              <TimelineLink
                id={`project-${project.ref_id}`}
                leftmargin={betterLeftMargin}
                width={betterWidth}
                to={`/app/workspace/apps/projects/${project.ref_id}`}
              >
                <ProjectStatusTag status={project.status} format="icon" />
                <IsKeyTag isKey={project.is_key} />
                <OverdueSign today={today} project={project} />
                <EntityNameOneLineComponent
                  name={`[${projectDonePct(
                    project,
                    projectStatsByRefId.get(project.ref_id)!,
                  )}%] ${project.name}`}
                />
              </TimelineLink>
            </SmallScreenTimelineLine>
...
function OverdueSign({ today, project }: OverdueSignProps) {
  ...
  const theDueDate = aDateToDate(project.due_date);
