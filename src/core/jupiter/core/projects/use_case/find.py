...
class ProjectFindUseCase(
    ...
    async def _perform_transactional_read(
        ...
        filter_status: list[ProjectStatus] | NoFilter = (
            ProjectStatus.all_workable_statuses()
        ...
        if include_inbox_tasks:
            inbox_tasks = await uow.get_for(InboxTask).find_all_generic(
                ...
                allow_archived=True,
                owner=[
                    EntityLink.std(NamedEntityTag.PROJECT.value, bp.ref_id)
                    for bp in projects
                ],
            )
        else:
        ...
        if include_notes:
            ...
            notes = await uow.get(NoteRepository).find_all_for_note_collection(
                ...
                allow_archived=True,
                filter_owners=[
                    EntityLink.std(NamedEntityTag.PROJECT.value, rid)
                    for rid in [bp.ref_id for bp in projects]
                ],
            )
            for note in notes:
        ...
        # Load contacts linked to projects
