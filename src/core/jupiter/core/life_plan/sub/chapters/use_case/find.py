...
@use_case_args
class ChapterFindArgs(JupiterFindCrownEntityArgs):
    """ChapterFindArgs."""
...
@use_case_result
class ChapterFindResultEntry(UseCaseResultBase):
...
@use_case_result
class ChapterFindResult(UseCaseResultBase):
    """ChapterFindResult object."""

    entries: list[ChapterFindResultEntry]


@readonly_use_case(WorkspaceFeature.LIFE_PLAN)
class ChapterFindUseCase(
    JupiterFindCrownEntityUseCase[ChapterFindArgs, ChapterFindResult]
):
    """The command for finding chapters."""
    ...
    async def _perform_transactional_read(
        ...
        if args.filter_ref_ids is not None:
            accessible_set = set(accessible_chapter_ref_ids)
            accessible_chapter_ref_ids = [
                ref_id for ref_id in args.filter_ref_ids if ref_id in accessible_set
            ]
        if not accessible_chapter_ref_ids:
            return ChapterFindResult(entries=[])
        ...
        notes_by_chapter_ref_id: defaultdict[EntityId, Note] = defaultdict(None)
        if include_notes:
            ...
            notes = await uow.get(NoteRepository).find_all_for_note_collection(
                ...
                allow_archived=True,
                filter_owners=[
                    EntityLink.std(NamedEntityTag.CHAPTER.value, rid)
                    for rid in [c.ref_id for c in chapters]
                ],
            )
            for note in notes:
                notes_by_chapter_ref_id[note.owner.ref_id] = note
        ...
        if include_tags:
            ...
            tag_links = await uow.get(TagLinkRepository).find_all_generic(
                ...
                owner=[
                    EntityLink.std(NamedEntityTag.CHAPTER.value, c.ref_id)
                    for c in chapters
                ],
            )
