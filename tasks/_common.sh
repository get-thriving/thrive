...
run_jupiter_webapp() {
    ...
    location_resolver=${location_resolver:-${LOCATION_RESOLVER:-noop}}
    ...
    if [[ "$location_resolver" != "noop" && "$location_resolver" != "google-maps" ]]; then
        log error "Invalid location resolver: $location_resolver (expected noop or google-maps)"
        exit 1
    fi
    ...
    log info "Running Jupiter WebApi in universe: $UNIVERSE, instance: $INSTANCE, webapi port: $WEBAPI_PORT, webapi postgres port: $WEBAPI_POSTGRES_PORT, api port: $API_PORT, webui port: $WEBUI_PORT, published port: $PUBLISHED_PORT, docs port: $DOCS_PORT, mcp port: $MCP_PORT, webapi blend (ADR 0008) storage=$webapi_storage_engine telemetry=$webapi_telemetry search=$webapi_search location_resolver=$location_resolver crm=$webapi_crm auth_provider=$webapi_auth_provider email_verification_strategy=$email_verification_strategy email_sender=$webapi_email_sender, source: $source, version: $version, mode: $mode"

    if [[ "$UNIVERSE" == "dev" ]]; then
        if [[ "$mode" == "pm2" ]]; then
            _run_dev_jupiter_webapp_with_pm2 "$INSTANCE" "$WEBAPI_PORT" "$WEBAPI_POSTGRES_PORT" "$API_PORT" "$WEBUI_PORT" "$PUBLISHED_PORT" "$DOCS_PORT" "$MCP_PORT" "$should_wait" "$should_monit" "$in_ci" "$source" "$version" "$clear_first" "$webapi_storage_engine" "$webapi_telemetry" "$webapi_search" "$webapi_crm" "$webapi_auth_provider" "$webapi_email_sender" "$email_verification_strategy" "$run_all_crons" "$webapi_postgres_url" "$location_resolver"
        else
            _run_dev_jupiter_webapp_with_docker "$INSTANCE" "$WEBAPI_PORT" "$WEBAPI_POSTGRES_PORT" "$API_PORT" "$WEBUI_PORT" "$PUBLISHED_PORT" "$DOCS_PORT" "$MCP_PORT" "$should_wait" "$should_monit" "$in_ci" "$source" "$version" "$clear_first" "$webapi_storage_engine" "$webapi_telemetry" "$webapi_search" "$webapi_crm" "$webapi_auth_provider" "$webapi_email_sender" "$email_verification_strategy" "$run_all_crons" "$webapi_postgres_url" "$location_resolver"
        fi
    elif [[ "$UNIVERSE" == "thrive-sh-test" ]]; then
        _run_thrive_sh_test_webapp "$INSTANCE" "$WEBAPI_PORT" "$WEBAPI_POSTGRES_PORT" "$API_PORT" "$WEBUI_PORT" "$PUBLISHED_PORT" "$DOCS_PORT" "$MCP_PORT" "$should_wait" "$should_monit" "$in_ci" "$source" "$version" "$clear_first" "$webapi_storage_engine" "$webapi_telemetry" "$webapi_search" "$webapi_crm" "$webapi_auth_provider" "$webapi_email_sender" "$email_verification_strategy" "$location_resolver"
    else
...
_run_dev_jupiter_webapp_with_pm2() {
    ...
    export LOCATION_RESOLVER="$location_resolver"
    ...
    if [[ "$source" == "registry" ]]; then
        ...
        gcloud compute ssh "$gcp_vm_name" \
            ...
            --command "bash -c '
                ...
                echo \"LOCATION_RESOLVER=${location_resolver}\" >> .env &&
...
write_jupiter_run_webapi_env() {
    ...
    mkdir -p "$RUN_ROOT/$instance"
    {
        ...
        printf '%s=%q\n' LOCATION_RESOLVER "${LOCATION_RESOLVER:-noop}"
        printf '%s=%q\n' CRM "${CRM:-noop}"
