...
if ! jupiter_postgres_server_reachable "$POSTGRES_HOST" "$POSTGRES_PORT" "$POSTGRES_USER" "$POSTGRES_PASSWORD" "$POSTGRES_DB"; then
    ...
    log error "Start the Postgres sidecar (instance apps should be stopped during import): mise run run:srv --instance ${instance} --storage-engine postgres"
    exit 1
fi

uri="$(jupiter_postgres_libpq_uri "$POSTGRES_HOST" "$POSTGRES_PORT" "$POSTGRES_USER" "$POSTGRES_PASSWORD" "$POSTGRES_DB")"
admin_uri="$(jupiter_postgres_libpq_uri "$POSTGRES_HOST" "$POSTGRES_PORT" "$POSTGRES_USER" "$POSTGRES_PASSWORD" "postgres")"
...
log info "Stop the instance before import so nothing reconnects mid-restore."
...
log info "Dropping and recreating database ${POSTGRES_DB}…"
PGPASSWORD="${POSTGRES_PASSWORD}" psql "$admin_uri" -v ON_ERROR_STOP=1 -q <<SQL
...
if [[ -n "$dump_dir" ]]; then
    ...
    log info "Restoring with pg_restore (--exit-on-error --no-owner --no-privileges --jobs=4)…"
    ...
    log info "Restoring with psql (ON_ERROR_STOP=1)…"
    PGPASSWORD="${POSTGRES_PASSWORD}" psql "$uri" -v ON_ERROR_STOP=1 -f "$sql_dump"
else
...
log info "Syncing ref_id sequences to match imported row ids…"
jupiter_postgres_sync_ref_id_sequences "$uri"

log info "Render export imported into instance ${instance} (${POSTGRES_DB} on ${POSTGRES_HOST}:${POSTGRES_PORT})."
log info "The dump carries production's alembic_version; compare it to your working tree and run migrations if needed."
